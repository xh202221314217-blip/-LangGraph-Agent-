...
def correct_cypher_query_relationship_direction(
    ...
    # 使用 langchain_neo4j 的 CypherQueryCorrector 根据图数据库 schema 修正关系方向
...
def validate_cypher_query_with_schema(
    ...
    nodes_and_rels = extract_entities_for_validation(cypher_statement=cypher_statement)#内有示例

    node_tasks = update_task_list_with_property_type( #为每个CypherValidationTask 任务分配property_type属性
        nodes_and_rels.get("nodes", list()), schema, "node"
    )
    '''
    [
        CypherValidationTask(labels_or_types="Person", operator="=", property_name="id", property_value="p123", property_type="STRING"),
        CypherValidationTask(labels_or_types="Person", operator="=", property_name="name", property_value="Alice", property_type="STRING"),
        CypherValidationTask(labels_or_types="Person", operator=">=", property_name="age", property_value="30", property_type="INTEGER"),
        CypherValidationTask(labels_or_types="Movie", operator="CONTAINS", property_name="title", property_value="Matrix", property_type="STRING"),
    ]
    '''

    rel_tasks = update_task_list_with_property_type( #为每个CypherValidationTask 任务分配property_type属性
        nodes_and_rels.get("relationships", list()), schema, "rel"
    )
    '''
    [
        CypherValidationTask(labels_or_types="ACTED_IN", operator="=", property_name="role", property_value="Neo", property_type="STRING"),
        CypherValidationTask(labels_or_types="ACTED_IN", operator="=", property_name="year", property_value="1999", property_type="INTEGER"),
    ]
    '''
...
def create_text2cypher_generation_node(
    ...
    async def generate_cypher(state: CypherInputState) -> Dict[str, Any]:
        ...
        """
        task = state.get("task", "")  #task中拆分出来的问题
...
def create_text2cypher_validation_node(
    ...
    async def validate_cypher(state: CypherState) -> Dict[str, Any]:
        ...
        if not llm_validation:
            cypher_errors = validate_cypher_query_with_schema(  # 基于schema校验节点/关系属性名、字符串枚举值和数值范围
                graph=graph, cypher_statement=cypher_statement
            )
