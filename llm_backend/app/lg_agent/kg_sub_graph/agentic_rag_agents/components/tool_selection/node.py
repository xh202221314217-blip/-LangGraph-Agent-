...
def create_tool_selection_node(
    ...
    async def tool_selection(
        ...
        if tool_selection_output is not None:
            ...
            """例如大模型返回 tool_calls=[{"name": "text2cypher", "args": {"task": state["question"]}}] 时，PydanticToolsParser 会把它解析成 text2cypher(task=...)。
            这里真实得到 tool_name == "text2cypher"。"""
            tool_args: Dict[str, Any] = tool_selection_output.model_dump() 
            """同一个 text2cypher 示例下，tool_selection_output.model_dump() 真实得到 tool_args == {"task": state["question"]}；如果选中其他工具，则得到该工具 schema 中定义的参数字典。"""
            #工具调用封装到了节点中。
            if tool_name == "predefined_cypher":
                return Command(
                    goto=Send(
                        "predefined_cypher",  #适合走预置 Cypher 模板，比如按类别查产品、查订单等。
                        {
            ...
            elif tool_name == "cypher_query":  #适合查结构化图数据库，比如产品价格、库存、规格。
                return Command(
                    goto=Send(
                        "cypher_query",
                        {
                ...
                return Command(
                    goto=Send(
                        "customer_tools",  #适合查文档型知识，比如故障、售后、保修、维修、退换货、评价等。
                        {
                            "task": state.get("question", ""),
                            "query_name": tool_name,
                            "query_parameters": tool_args,
            ...
            return go_to_text2cypher   #！！！！！！未定义

        # handle instance where no tool is chosen
        else:
            return Command(
                goto=Send(
                    "error_tool_selection",    #！！！节点没注册
                    {
                        "task": state.get("question", ""),
                        "errors": [
