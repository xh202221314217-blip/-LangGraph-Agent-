...
class NorthwindCypherRetriever(BaseCypherExampleRetriever):
    ...
    ORDER BY 销售额 DESC"""
        ...
        # 基本的相关性匹配
        def compute_relevance(example, query):   #！！！！应该升级为基于Embedding的相关性计算，而不是简单的关键词匹配
            # 通过关键词匹配
            score = 0
            ...
            for pattern, category in important_patterns:
                if re.search(pattern, query):
                    # 如果示例问题也包含相关模式，增加分数
                    if re.search(pattern, example["question"]):      #示例与问题具有相同关键词则得分
                        score += 3
                    
                    # 检查是否属于相关类别
                    for cat, cat_examples in all_examples.items():   #业务语义补偿，先找出问题所属的类别，依据该类别找出所有相关示例，如果example刚好在这些示例中，则增加分数
