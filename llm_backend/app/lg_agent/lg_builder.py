...
def route_query(
    ...
    if hasattr(state, "config") and state.config and state.config.get("configurable", {}).get("image_path"):  #！！！这个config貌似还没有正确传入state中，后续需要调整
...
async def create_image_query(
    ...
    if not api_key or not base_url or not vision_model:
        ...
        return {"messages": [AIMessage(content="未配置视觉模型，无法处理图片查询。请联系管理员。")]}
    ...
    try:
        ...
        with Image.open(image_path) as img:
            # 设置最大尺寸
            max_size = 1024    #宽和高都不超过1024像素，保持原图宽高比
            # 计算缩放比例
            width, height = img.size        #获取原图尺寸
            ratio = min(max_size / width, max_size / height)  #计算图片应该缩小多少倍，保证缩放后宽高都不超过 max_size。
            ...
            img_byte_arr.seek(0)  #上面刚执行完save操作后，BytesIO对象的指针在末尾，需要重置到开头才能正确读取数据
