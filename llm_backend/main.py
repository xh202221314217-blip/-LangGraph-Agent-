...
async def langgraph_query(
    ...
    try:
        ...
        if image:
            ...
            # 生成带时间戳的文件名
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            original_name, ext = os.path.splitext(image.filename)  #路径攻击风险：如果用户上传的文件名包含路径分隔符（如../），可能会导致文件被保存到不安全的位置。
            new_filename = f"{original_name}_{timestamp}{ext}"
            ...
            async def process_stream():
                async for c, metadata in graph.astream(    #这种情况是处理interrupt中断后续的继续执行
                    ...
                    if c.content and "research_plan" not in metadata.get("tags", []) and not c.additional_kwargs.get("tool_calls"):
                        #如果c.content有内容，并且tags标签表示不在research_plan中，并且没有工具调用，则认为这是最终展示给用户的内容
                    ...
                    # 工具调用单独处理，不发送给前端
                        ...
                        tool_data = c.additional_kwargs.get("tool_calls")[0]["function"].get("arguments")   #！！！旧写法，而且流式调用可能拿到不完整的arguments！！！！！！！！！！！！！！！
